...
//yq				str1+="<table>"; 
			...
			str1+="<tr>";
 			str1+="<td><div><a href='performPage.action?projectId="+val.project.id+"&advance="+val.advance+"' title='点击可查询详细信息'>"+val.project.projectname+"</a></div></td><td>"+dateToStr(date)+"</td>";		
 			str1+="</tr>";
//yq			str1+="</table>";
				});
			var up=num-1;
			...
			if(endPage<=1){
			str1_page+="<font color='#e0e0e0'>首页 上一页 下一页 最后一页</font>&nbsp;&nbsp;";
			}else{
			...
			str1_page+="  <a href='javascript:void(0);' onclick='fb("+endPage+")'>最后一页</a>&nbsp;&nbsp;" ;
			}
			$("#oo").append(str1);
...
//yq				str1+="<table>"; 
				...
				num1=val.d;
				str1+="<tr>";
				str1+="<td><a href='performPage.action?projectId="+val.url.task.project.id+"&urlId="+val.url.id+"&taskId="+val.url.task.id+"'title='点击可查询详细信息'>"+val.url.task.project.projectname+"</a></td><td>"+val.url.task.userUpper.realname+"</td><td><a href='${pageContext.request.contextPath}/down.do?resourceName="+val.url.docUrl+"'>"+val.url.docUrl+"</a></td>";
//yq 				str1+="</tr>";
//yq				str1+="</table>";
				});
			var up=num-1;
			...
			if(endPage<=1){
			str1_page+="<font color='#feeeee'>首页 上一页 下一页 最后一页</font>&nbsp;&nbsp;";
			}else{	
			...
			str1_page+="  <a href='javascript:void(0);' onclick='rw("+endPage+")'>最后一页</a>" ;
			}
			$("#bb").append(str1);
...
<table width="100%" border="0" cellpadding="1" cellspacing="1" >
    ...
    <tbody>
		<s:iterator value="workUrl" var="url">
             <tr>
                <td><a href="performPage.action?projectId=${url.task.project.id}&urlId=${url.id}&taskId=${url.task.id}" title="点击可查询详细信息">${url.task.project.projectname}</a></td>
                <td>${url.task.userUpper.realname}</td>
...
<table width="100%" border="0" cellpadding="1" cellspacing="1" >
    ...
    <tbody>
            ...
            <td align="center">${bulletin.content}</td>
            </tr>
            </s:iterator>
    </tbody>
    <tr bgcolor="#DDDDEC">
