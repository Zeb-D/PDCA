...
<s:iterator value="countList" var="count">
        ...
        <td align="left">&nbsp;<a href="down.do?resourceName=${count.url.docUrl }">${count.url.docUrl}</a></td>   
        <td><a href="selectTime.action?gotoid=${count.url.id}">具体详情${count.url.id}</a></td>   
        <td>
        <input
							type="button" value="更改" class="button2" onclick="javascript:location.href='changeplan.action?updateId=${count.url.id}' "/>
        <input type="hidden" value="${count.url.id}"> <input
							type="button" id="deleteId" value="删除" class="button2" />
        </td>   
        </tr>
        </s:iterator>
    <tr bgcolor="#DDDDEC">
