...
<style>
    ...
    .calendar dd{clear: both;width: 183px;height: 139px;font-weight: 700;background:url(style/images/o_bg.png) no-repeat; margin:0;}
    .prevyear,.nextyear, .prevmonth,.nextmonth{cursor:pointer;height:9px; background:url(style/images/o_nextprv.png) no-repeat; overflow:hidden;position:absolute; top:8px; text-indent:-999px;}
    ...
    .input{ border:1px solid #ccc; padding:4px; background:url(style/images/o_nextprv.png) no-repeat right -18px;}
</style>
	...
	$.ajax({	
			...
			success : function(data){
				...
				$.each(data.p1, function(i,d) {
					input.appendChild(document.createTextNode((i+1)+"."));
					var a = document.createElement("a");
					a.setAttribute("href","${pageContext.request.contextPath}/down.do?resourceName="+d.docName);
					a.setAttribute("text-decoration","none");
					a.appendChild(document.createTextNode(d.docName+"　"));
...
var a = [];
function validate1() {
	var username = $("#selectname1").find("option:selected").text();
	if(username!="请选择"){
	$("#inpt").append("<div id="+username+" style=\"float:left\">"+username+"<input id=\"btn1\" type=\"button\" value=\"X\" onclick=\"Del('"+username+"')\"/></div>");
	a.push(username);
	}

	/*var userList = document.getElementById("inpt").childNodes;
	for(var i = 0;i<userList.length;i++){
				if(userList[i].textContent!="")
					//a.push(userList[i].textContent);
...
function changefile(){
	var input = document.getElementById("input");
	var a = document.createElement("a");
	a.setAttribute("href","${pageContext.request.contextPath}/down.do?resourceName="+$("#fileUpload").val());
	a.setAttribute("text-decoration","none");
	a.appendChild(document.createTextNode($("#fileUpload").val()+"　"));
					//var br = document.createElement("br");
					//a.appendChild(br);
					input.appendChild(a);
}
      ...
      <td height="28" colspan="2" align="left">
            <table width="100%" border="0" cellpadding="0" cellspacing="3" class="table_show_input">
				<tr>
					...
					<td width="1155">职称: 
					  <s:select name="selectName" list="#listRole"
							listKey="rolename" listValue="rolename" theme="simple"
							id="selectName" headerValue=""    headerKey="请选择" onchange="return validate()">						</s:select>
				...
				<tr class="table_show_input">
				<td><strong>已选人：</strong></td>
				<td id="inpt">&nbsp;</td>
				</tr>
				<tr>
					<td><strong>文件模板：</strong></td>
					<td id="input">
					<%--<a href="${pageContext.request.contextPath}/down.do?resourceName=${document.docName}">${document.docName }</a>--%>					</td>
				</tr>
				<tr>
					<td><strong>文件上传：</strong></td>
					<td><s:file name="fileUpload" theme="simple" id="fileUpload" onchange="changefile()" />					</td>
				</tr>
				<tr >
				<td><strong>截止时间：</strong></td>
				<td>
					<!-- <input type="text" name="url.endtime" id="time"
						onfocus="c.showMoreDay = false;c.show(this);" /> -->
						<input size="25" name="url.endtime" type="text" id="j_Date1" class="input">					</td>
				</tr>
            </table>     
